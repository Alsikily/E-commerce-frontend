    ...
    <main class="home">
        <main class="body">
            <article class="products-container">
                ...
                <section class="products-table" v-if="customerProduct.orders.length > 0">
                    <div class="row">
                        <span>{{ $t('product-name') }}</span>
                        <span class="options">{{ $t('code') }}</span>
                    </div>
                    <div class="row" v-for="(order, index) in customerProduct.orders">
                        <span>
                        ...
                        <span class="options" v-if='order.paid !== 1'>
                            {{ order.verification_code }}
                        </span>
                        <span class="options" v-else>{{ $t('paid') }}</span>
                    </div>
                </section>
                <span class="data-not-found" v-else>{{ $t('no-data-found') }}</span>
            </article>
        </main>
    </main>
</template>
...
<script setup>
    ...
    await customerProduct.getMyOrders();
